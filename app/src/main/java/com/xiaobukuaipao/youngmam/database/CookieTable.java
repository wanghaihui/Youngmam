...
public class CookieTable {
    ...
    public static final String COLUMN_LOGIN_TYPE = "type";
    ...
    private static void updateTable(SQLiteDatabase database) {
        ...
        try {
            ...
            // rename the table
            String alterMessageTableSql = getAlterTableSql(TABLE_COOKIE);
            ...
            database.execSQL(DATABASE_CREATE);
            // 记得用旧表名
            String selectColumns = getColumnNames(database, TABLE_COOKIE + TEMP_SUFFIX);

            if (selectColumns.endsWith(",")) {
                selectColumns = selectColumns.substring(0, selectColumns.length() - 1);
            }

            String updateMessagesSql = "insert into " + TABLE_COOKIE + " ("
                    + selectColumns + ") "
                    + "select " + selectColumns + "" + " " + " from " + TABLE_COOKIE
                    + TEMP_SUFFIX;
            database.execSQL(updateMessagesSql);

            // drop the oldtable
            String dropMessageTableSql = getDropTableSql(TABLE_COOKIE + TEMP_SUFFIX);
