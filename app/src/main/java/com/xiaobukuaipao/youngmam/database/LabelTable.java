...
public class LabelTable {
    ...
    private static void updateTable(SQLiteDatabase database) {
        ...
        try {
            ...
            // rename the table
            String alterMessageTableSql = getAlterTableSql(TABLE_LABEL);
            ...
            database.execSQL(DATABASE_CREATE);
            // 记得用旧表名
            String selectColumns = getColumnNames(database, TABLE_LABEL + TEMP_SUFFIX);

            if (selectColumns.endsWith(",")) {
                selectColumns = selectColumns.substring(0, selectColumns.length() - 1);
            }

            String updateMessagesSql = "insert into " + TABLE_LABEL + " ("
                    + selectColumns + ") "
                    + "select " + selectColumns + "" + " " + " from " + TABLE_LABEL
                    + TEMP_SUFFIX;
            database.execSQL(updateMessagesSql);

            // drop the oldtable
            String dropMessageTableSql = getDropTableSql(TABLE_LABEL + TEMP_SUFFIX);
