 ...
 * 专门处理Activity, Fragment之间信息传递的基类--针对Article
 */
public abstract class BaseEventHttpFragment extends BaseHttpFragment implements OnFollowClickListener,
        OnActionClickListener, OnActionClickListener2, YoungShareBoard.OnShareSuccessListener {

    private static final String TAG = BaseEventHttpFragment.class.getSimpleName();
    ...
    public void initUIAndData() {
        ...
        article2Adapter = new Article2Adapter(this.getActivity(), articleList);
    ...
    public void onActionClick(final int action, final int position, final Article article) {
        if (action == OnActionClickListener.ACTION_LIKE) {
            ...
            if (sp.getLong(SplashActivity.UID, 0) > 0) {
                ...
                // 跳到登录页
                Intent intent = new Intent(this.getActivity(), RegisterAndLoginActivity.class);
            ...
            if (sp.getLong(SplashActivity.UID, 0) > 0) {
                // 评论文章
                Intent intent = new Intent(this.getActivity(), FreshDetailsActivity.class);
            ...
            YoungShareBoard shareBoard = new YoungShareBoard(this.getActivity());
            shareBoard.setShareParams(article.getArticleId(), article.getBusinessType());
            shareBoard.setOnShareSuccessListener(this);
            shareBoard.showAtLocation(this.getActivity().getWindow().getDecorView(), Gravity.BOTTOM, 0, 0);
            ...
            mController.setShareMedia(new UMImage(this.getActivity(), imageUrl));
        }
    }
